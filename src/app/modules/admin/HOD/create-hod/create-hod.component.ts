...
@Component({
  ...
  templateUrl: './create-hod.component.html',
  styleUrl: './create-hod.component.scss'
})
export class CreateHODComponent {
  ...
  constructor(
    ...
    public matDialogRef: MatDialogRef<CreateHODComponent>,
  ...
  ngOnInit(): void {
    ...
    this._HODservice.getDesignation().subscribe(res=>{
      this.designationList =res;
    })
    this.lecturerForm.get('CourseYear').valueChanges.subscribe(res=>{
      this._HODservice.getSubjectbyAcademicYear(res.guid).subscribe(res=>{
        this.subjects = res;
      })
    })
    ...
    // Load course list
    this._HODservice.cousreList().subscribe((response: any) => {
      this.cousreDetails = response;
    });

    // Load subjects
    this._HODservice.qbankTypeList(QBankCategory.General).pipe(
      tap(response => {
    ...
    // Load course years
    this._HODservice.getCourseYear().pipe(
      tap(response => {
    ...
    if (this.action === 'edit' && this._data.lecturer) {
      ...
      combineLatest([coursesLoaded$, subjectsLoaded$]).pipe(
        ...
        this.lecturerForm.patchValue({
          ...
          signature: this._data.lecturer.signature
        });
  ...
  //     Password: ['', this.action === 'edit' ? [] : [Validators.required, Validators.minLength(6)]]
  //   });
  //   this._HODservice.cousreList().subscribe((responce: any) => {
  //     this.cousreDetails = responce;

  //   });
  //   this._HODservice.qbankTypeList(QBankCategory.General).subscribe((responce: any) => {
  //     this.subjectDetails = responce;

  //   });

  //   this._HODservice.getCourseYear().subscribe((responce: any) => {
  ...
  // getCourseYear(guid) {
  //   this._HODservice.getCourseYaerByCousreGuid(guid).then((response: any) => {
  ...
  onselectFile(e: any) {
    ...
    if (file) {
      ...
      this._HODservice.uploadImage(file).subscribe((response: any) => {
  ...
  onSave() {
    ...
    if (this.lecturerForm.invalid) {
      ...
      if (this.action === 'edit') {
        // Call update API
        this._HODservice.updateLecturer(request).then((res: any) => {
          if (res) {
        ...
        // Call create API
        this._HODservice.createLecture(request).then((res: any) => {
